...
import {
  ...
  statusLabel,
} from "@/lib/assessment/movement-definitions";
...
export function GuidedAssessment() {
  ...
  function handleFrame(nextFrame: PoseFrame | null) {
    ...
    const normalized = angles.map((item) => ({ ...item, frameTimestamp: item.frameTimestamp - start }));
    ...
    if (definition.repCounting === "side-knee-cycle") {
      if (!segmenterRef.current) {
        const knee = normalized
          .filter((item) => item.angleName === "leftKneeFlexion" || item.angleName === "rightKneeFlexion")
          .sort((a, b) => b.confidence - a.confidence)[0];
        if (knee) segmenterRef.current = new RepSegmenter({ ...DEFAULT_KNEE_REP_CONFIG, angleName: knee.angleName });
      }
  ...
  function beginCountdown() {
    if (!isCalibrated || countdown !== null) return;
    setCountdown(3);
  }
    ...
    <div className="mx-auto max-w-7xl px-4 pb-16 pt-6 sm:px-6 lg:px-8">
      ...
      {step === 1 && (
        <section className="mx-auto max-w-5xl py-8 sm:py-12">
          ...
          <p className="mt-3 max-w-2xl text-base leading-7 text-zinc-600">Choose the movement and camera view that match what you want to measure. Each card only lists measurements the current camera pipeline can support.</p>
            ...
            {MOVEMENT_DEFINITIONS.map((item) => (
              <button key={item.id} type="button" onClick={() => chooseMovement(item.id)} className="group rounded-[1.75rem] bg-white p-6 text-left shadow-sm ring-1 ring-zinc-200 transition hover:-translate-y-0.5 hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-sky-500">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="text-xl font-semibold text-zinc-950">{item.title}</p>
                    <p className="mt-1 text-sm font-medium text-sky-700">{item.viewLabel}</p>
                  </div>
      ...
      {needsCamera && (
        <section className="mt-6 space-y-6">
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div>
              ...
              <h1 className="mt-2 text-2xl font-semibold text-zinc-950 sm:text-3xl">
                {step === 2 ? "Set up your camera" : step === 3 ? "Calibrate your position" : "Complete the movement"}
              </h1>
          ...
          {step === 2 && (
            <div className="flex flex-col gap-4 rounded-[1.75rem] bg-white p-5 shadow-sm ring-1 ring-zinc-200 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p className="font-semibold text-zinc-950">Camera connection</p>
                <p className="mt-1 text-sm text-zinc-600">{frame ? "Pose tracking is active. Continue to calibration." : "Enable the camera and move into view before continuing."}</p>
              </div>
                ...
                <div className="mt-4 grid gap-2 sm:grid-cols-2">
                  {calibrationChecks.map((check) => (
                    <div key={check.id} className="flex items-center gap-2 text-sm text-zinc-700">
                      ...
                      <span className="sr-only">{check.passed ? "passed" : "not yet passed"}</span>
                    </div>
                  ))}
                </div>
          ...
          {step === 4 && (
            <div className="flex flex-col gap-4 rounded-[1.75rem] bg-zinc-950 p-5 text-white shadow-xl sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.18em] text-white/55">Recording</p>
                <p className="mt-1 text-lg font-semibold">Move naturally through the assessment.</p>
                <p className="mt-1 text-sm text-white/65">{captureReady ? "Capture quality is currently usable." : "Tracking quality dropped. Reposition before continuing."}</p>
              </div>
      ...
      {step === 5 && <ResultsScreen key={resultsVersion} movement={movement} readings={readingsRef.current} reps={repsRef.current} telemetry={telemetry} selectedRep={selectedRep} onSelectRep={setSelectedRep} onRestart={restart} />}
    </div>
  );
}
...
function ResultsScreen({ movement, readings, reps, telemetry, selectedRep, onSelectRep, onRestart }: {
  ...
  const selectedTrajectory = selected
    ? primary.filter((item) => item.frameTimestamp >= selected.startedMs && item.frameTimestamp <= selected.endedMs)
    : [];
  ...
  return (
    <section className="mx-auto max-w-6xl py-8 sm:py-12">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="text-sm font-semibold uppercase tracking-[0.2em] text-sky-700">Movement summary</p>
          <h1 className="mt-3 text-3xl font-semibold tracking-tight text-zinc-950 sm:text-4xl">{definition.title} · {definition.viewLabel}</h1>
          <p className="mt-3 max-w-2xl text-sm leading-6 text-zinc-600">Camera-derived movement measurements from this session. This is not a medical diagnosis or injury prediction.</p>
        </div>
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <SummaryMetric label="Completed repetitions" value={definition.repCounting === "side-knee-cycle" ? String(reps.length) : "Not enabled"} note={definition.repCounting === "side-knee-cycle" ? "Validated engineering detector for this capture mode" : "Rep detector not yet validated for this movement"} />
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px]">
        <div className="rounded-[1.75rem] bg-white p-5 shadow-sm ring-1 ring-zinc-200">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-sm font-semibold text-zinc-950">Measurement trajectory</p>
              <p className="mt-1 text-xs text-zinc-500">{selected ? `Rep ${selected.repIndex}` : ANGLE_LABELS[definition.primaryMetric]}</p>
            </div>
            {selected && <button type="button" onClick={() => onSelectRep(null)} className="text-xs font-semibold text-zinc-600 underline">Show full session</button>}
          </div>
          ...
          {selected && (
            <dl className="mt-4 grid grid-cols-3 gap-3 border-t border-zinc-100 pt-4 text-sm">
              <div><dt className="text-zinc-500">Excursion</dt><dd className="mt-1 font-semibold text-zinc-950">{selected.excursionDegrees.toFixed(1)}°</dd></div>
              <div><dt className="text-zinc-500">Peak</dt><dd className="mt-1 font-semibold text-zinc-950">{selected.peakValue.toFixed(1)}°</dd></div>
              <div><dt className="text-zinc-500">Duration</dt><dd className="mt-1 font-semibold text-zinc-950">{((selected.endedMs - selected.startedMs) / 1000).toFixed(2)} s</dd></div>
            </dl>
          )}
        </div>
        ...
        <div className="space-y-4">
          <div className="rounded-[1.75rem] bg-zinc-950 p-5 text-white">
            <p className="text-sm font-semibold">Capture quality</p>
            <dl className="mt-4 space-y-3 text-sm">
              <Row label="Resolution" value={telemetry?.width && telemetry.height ? `${telemetry.width}×${telemetry.height}` : "Unavailable"} />
              <Row label="Camera FPS" value={telemetry?.cameraFps ? telemetry.cameraFps.toFixed(1) : "Unavailable"} />
              <Row label="Pose FPS" value={telemetry?.inferenceFps ? telemetry.inferenceFps.toFixed(1) : "Unavailable"} />
      ...
      {reps.length > 0 && (
        <div className="mt-6 rounded-[1.75rem] bg-white p-5 shadow-sm ring-1 ring-zinc-200">
          <p className="font-semibold text-zinc-950">Repetitions</p>
          <p className="mt-1 text-sm text-zinc-600">Select a repetition to inspect its trajectory and measured excursion.</p>
          <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {reps.map((rep) => <button key={rep.repIndex} type="button" onClick={() => onSelectRep(rep.repIndex)} className={`rounded-2xl p-4 text-left ring-1 ${selectedRep === rep.repIndex ? "bg-sky-50 ring-sky-300" : "bg-zinc-50 ring-zinc-200"}`}><p className="font-semibold text-zinc-950">Rep {rep.repIndex}</p><p className="mt-2 text-sm text-zinc-600">Excursion {rep.excursionDegrees.toFixed(1)}°</p><p className="text-sm text-zinc-600">Duration {((rep.endedMs - rep.startedMs) / 1000).toFixed(2)} s</p></button>)}
          </div>
        </div>
      )}
    </section>
...
function SummaryMetric({ label, value, note }: { label: string; value: string; note: string }) {
  return <div className="rounded-[1.5rem] bg-white p-5 shadow-sm ring-1 ring-zinc-200"><p className="text-xs font-semibold uppercase tracking-[0.14em] text-zinc-500">{label}</p><p className="mt-3 text-2xl font-semibold text-zinc-950">{value}</p><p className="mt-2 text-xs leading-5 text-zinc-500">{note}</p></div>;
}

function Row({ label, value }: { label: string; value: string }) {
  return <div className="flex items-center justify-between gap-4"><dt className="text-white/55">{label}</dt><dd className="font-medium">{value}</dd></div>;
}
