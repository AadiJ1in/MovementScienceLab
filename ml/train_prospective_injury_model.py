...
Cohort timing/leakage, pre-specified data adequacy, a machine-readable
outcome-specific protocol, and predictor evidence metadata must pass before the
nested benchmark is allowed to run.
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Audit a pre-specified prospective research protocol, cohort timing, data adequacy, and predictor "
            "evidence policy before running the nested participant-grouped injury benchmark."
        )
    )
